 ...
 * (`variant="page"`) e no pop-up da lista (`variant="modal"`).
 */
export async function TaskDetail({
  ...
  if (!task) return modal ? <Missing /> : notFound();
              ...
              <DeleteButton
                ...
                confirm={
                  task.children.length > 0
                    ? "Excluir este marco? As tarefas dele ficam sem marco; impedimentos, avaliações e histórico do marco são apagados."
                    : "Excluir esta tarefa? Impedimentos, avaliações e histórico dela são apagados."
                }
                returnTo={modal ? undefined : `/projects/${id}`}
                label="Excluir"
              />
            ) : null}
          </div>
