...
/**
 * Lista de reuniões — do projeto (mais recentes primeiro) ou, com
 * `showProject`, da carteira inteira (`/meetings`), com a coluna Projeto e
 * o link de cada linha apontando para o projeto dela em vez de `projectId`.
 */
export function MeetingTable({
  ...
  meetings: MeetingRow[];
  now: Date;
  /** Obrigatório quando `showProject` é falso (lista dentro de um projeto). */
  projectId?: string;
  showProject?: boolean;
}) {
  ...
  return (
    <div className="dp-scroll dp-box dp-box-canvas max-h-[30rem] rounded-lg border border-line bg-surface">
      <ResizableTable id={showProject ? "meetings-table-portfolio" : "meetings-table"} className="w-full border-collapse">
        ...
        <tbody>
          {meetings.map((m) => {
            ...
            const href = showProject && m.project
              ? `/projects/${m.project.id}/meetings/${m.id}`
              : `/projects/${projectId}/meetings/${m.id}`;
            return (
              <ClickableRow
                key={m.id}
                href={href}
              ...
              >
                ...
                {showProject ? (
                  <Td className="truncate text-ink-soft">{m.project?.name ?? "—"}</Td>
                ) : null}
